...
*/
l_sinline int
LTintfloat(lua_Integer i, lua_Number f)
{
  if (l_intfitsf(i))
...
*/
l_sinline int
LEintfloat(lua_Integer i, lua_Number f)
{
  if (l_intfitsf(i))
...
*/
l_sinline int
LTfloatint(lua_Number f, lua_Integer i)
{
  if (l_intfitsf(i))
...
*/
l_sinline int
LEfloatint(lua_Number f, lua_Integer i)
{
  if (l_intfitsf(i))
...
*/
l_sinline int
LTnum(const TValue* l, const TValue* r)
{
...
*/
l_sinline int
LEnum(const TValue* l, const TValue* r)
{
  lua_assert(ttisnumber(l) && ttisnumber(r));
  if (ttisinteger(l)) {
    lua_Integer li = ivalue(l);
...
*/
static int
lessthanothers(lua_State* L, const TValue* l, const TValue* r)
{
  lua_assert(!ttisnumber(l) || !ttisnumber(r));
...
*/
static int
lessequalothers(lua_State* L, const TValue* l, const TValue* r)
{
  lua_assert(!ttisnumber(l) || !ttisnumber(r));
  if (ttisstring(l) && ttisstring(r)) /* both are strings? */
    return l_strcmp(tsvalue(l), tsvalue(r)) <= 0;
...
*/
int
luaV_lessequal(lua_State* L, const TValue* l, const TValue* r)
{
  if (ttisnumber(l) && ttisnumber(r)) /* both operands are numbers? */
    return LEnum(l, r);
  else
